    ...
    (set, get) => ({
      ...
      addOrCreateUserMessage: (activeId, content) => {
        ...
        if (!activeId) {
          ...
          set((s) => ({
            conversations: [{ id, title, messages: [userMsg], updatedAt: Date.now() }, ...s.conversations],
        ...
        set((s) => ({
          conversations: s.conversations.map((c) =>
            c.id === activeId ? { ...c, messages: [...c.messages, userMsg], updatedAt: Date.now() } : c,
          ),
        }));
        return activeId;
      },
      ...
      addAssistantMessage: (conversationId, content) => {
        ...
        set((s) => ({
          conversations: s.conversations.map((c) =>
            c.id === conversationId ? { ...c, messages: [...c.messages, assistantMsg], updatedAt: Date.now() } : c,
          ),
        }));
      },
      ...
      deleteConversation: (id) => {
        set((s) => {
          ...
          const active = s.activeConversationId === id ? null : s.activeConversationId;
